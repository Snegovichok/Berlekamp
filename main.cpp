...
void run() {
    ...
    vector<vector<Polynomial>> allAdjustedPolys(MOD);

    // Перебор значений для minus от 0 до MOD
    for (int i = 0; i < MOD; ++i) {
        cout << "вычитаем " << i << endl;
        for (const auto& poly : polys) {
            Polynomial adjustedPoly = poly.subtract(Polynomial(vector<int>{i}));
    ...
    for (const auto& row : ResultPolyMatrix) {
        ...
        counter++;
        if (counter % MOD == 0) { // если достигнуто MOD решений, выводим перевод строки
            cout << endl;
        }
    }
}
